...
fn detect_upkg_glibc(prefix_dir: &Path) -> Option<PathBuf> {
...
fn patch_elf_placeholders(keg_path: &Path, prefix_dir: &Path) -> Result<(), Error> {
    ...
    let upkg_interpreter = detect_upkg_glibc(prefix_dir);

    let target_interpreter = if let Some(ref upkg_ld) = upkg_interpreter {
        Some(upkg_ld.clone())
    } else {
        find_system_ld_so()
    };
...
mod tests {
    ...
    fn test_glibc_detection() {
        ...
        assert!(detect_upkg_glibc(&prefix).is_none());
        ...
        let detected = detect_upkg_glibc(&prefix);
