...
pub fn run_init(root: &Path, prefix: &Path, no_modify_path: bool) -> Result<(), InitError> {
    ...
    println!("{} Initializing upkg-native...", style("==>").cyan().bold());
...
const UPKG_BLOCK_START: &str = "# >>> upkg-native >>>";
const UPKG_BLOCK_END: &str = "# <<< upkg-native <<<";

fn upsert_managed_block(existing: &str, managed_block: &str) -> String {
    if let Some(start_idx) = existing.find(UPKG_BLOCK_START)
        && let Some(end_rel_idx) = existing[start_idx..].find(UPKG_BLOCK_END)
    {
        let mut end_idx = start_idx + end_rel_idx + UPKG_BLOCK_END.len();
...
"#,
        ...
        println!(
            "    {} To use upkg-native, add {} and {} to your PATH",
            style("→").cyan(),
            upkg_bin,
            prefix_bin.display()
        );
...
pub fn ensure_init(root: &Path, prefix: &Path, auto_init: bool) -> Result<(), crate::types::Error> {
    ...
    if is_interactive && !auto_init {
        println!(
            "{} upkg-native needs to be initialized first.",
...
mod tests {
    ...
    fn add_to_path_writes_core_env_vars_with_guarded_ca_setup() {
        ...
        let upkg_dir = "/home/user/.upkg-native";
        let upkg_bin = "/home/user/.upkg-native/bin";
        ...
        assert!(content.contains("export UPKG_DIR=/home/user/.upkg-native"));
        assert!(content.contains("export UPKG_BIN=/home/user/.upkg-native/bin"));
    ...
    fn shell_config_file_selection(
        ...
        assert!(content.contains("# upkg-native"));
    }
    ...
    fn upsert_managed_block_replacement_consumes_trailing_newline() {
        ...
        assert!(first.contains("# <<< upkg-native <<<\npostfix\n"));
        assert!(!first.contains("# <<< upkg-native <<<\n\npostfix\n"));
    }
}
