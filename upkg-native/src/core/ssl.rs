...
pub fn find_ca_bundle(prefix: &Path) -> Option<PathBuf> {
    let cellar_dir = prefix.join("Cellar").join("ca-certificates");
    let entries = std::fs::read_dir(&cellar_dir).ok()?;

    for entry in entries.flatten() {
        let keg_path = entry.path();
        if !keg_path.is_dir() {
            continue;
        }
        let candidates = [
